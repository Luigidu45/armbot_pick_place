...
# ═══════════════════════════════════════════════════════
# CONTROLADOR CON PICK & PLACE INTEGRADO
# ═══════════════════════════════════════════════════════
class RobotGUIControllerDual(Node):
    def __init__(self):
        ...
        self.yolo_position_sub = self.create_subscription(
            Point, '/yolo/object_position', self.yolo_position_callback, 10)
        self.yolo_class_sub = self.create_subscription(
            String, '/yolo/object_class', self.yolo_class_callback, 10)
        ...
        self.pickplace_start_time = 0
        
        # Calibración YOLO
        self.yolo_x_far = 169.4
        self.yolo_x_near = 57.6
        self.yolo_y_left = 16.3
        self.yolo_y_right = 223.6
        self.m1_left_limit = -30.3
        self.m1_right_limit = 27.3
        
        self.ground_points = np.array([
            [-90.0, 25.0], [-78.8, 50.0], [-74.0, 52.4],
            [-71.7, 56.7], [-57.0, 90.0]
        ])
        self.ground_distances = np.linspace(1.0, 0.0, len(self.ground_points))
        
        self.bin_positions = {'Batería': 90.0, 'Motor': -90.0, 'Placa': 90.0}
        self.lift_height_m2 = -20.0
        self.lift_height_m3 = 10.0
        
        # Variables temporales para pick&place
        self.target_m1_pp = 0.0
        self.target_m2_pp = 0.0
        self.target_m3_pp = 0.0
        ...
        self.create_timer(0.1, self.pickplace_state_machine)  # ← NUEVO
        
        if self.arduino_connected: 
            threading.Thread(target=self.serial_read_loop, daemon=True).start()
        
        self.get_logger().info('🤖 Robot Controller con Pick&Place iniciado')
    ...
    def yolo_position_callback(self, msg):
        self.object_x = msg.x
        self.object_y = msg.y
        self.get_logger().info(f'📍 YOLO Posición: x={msg.x:.1f}, y={msg.y:.1f}', throttle_duration_sec=2.0)
    
    def yolo_class_callback(self, msg):
        self.object_class = msg.data
        self.get_logger().info(f'🏷️ YOLO Clase: {msg.data}', throttle_duration_sec=2.0)
        
        # Si estamos en IDLE y hay detección válida, iniciar
        if (self.pickplace_state == PickPlaceState.IDLE and 
            self.object_class in self.bin_positions and
            self.object_x is not None and self.object_y is not None):
            
            self.get_logger().info(f'✅ Iniciando Pick&Place para {self.object_class}')
            self.pickplace_state = PickPlaceState.MOVING_TO_OBJECT
            self.pickplace_start_time = time.time()
            
            # Calcular ángulos
            angles = self.yolo_to_robot_angles(self.object_x, self.object_y)
            self.target_m1_pp, self.target_m2_pp, self.target_m3_pp = angles
            
            self.get_logger().info(f'🎯 Target: M1={self.target_m1_pp:.1f}° M2={self.target_m2_pp:.1f}° M3={self.target_m3_pp:.1f}°')

    # ═══════════════════════════════════════════════════════
    # INTERPOLACIÓN YOLO → ÁNGULOS
    # ═══════════════════════════════════════════════════════
    def yolo_to_robot_angles(self, x_yolo, y_yolo):
        distance_norm = np.interp(x_yolo, [self.yolo_x_near, self.yolo_x_far], [0.0, 1.0])
        m2 = np.interp(distance_norm, self.ground_distances, self.ground_points[:, 0])
        m3 = np.interp(distance_norm, self.ground_distances, self.ground_points[:, 1])
    ...
    # ═══════════════════════════════════════════════════════
    def pickplace_state_machine(self):
        if self.pickplace_state == PickPlaceState.DISABLED:
            return
        
        if self.pickplace_state == PickPlaceState.IDLE:
            return  # Esperando detección
        
        elif self.pickplace_state == PickPlaceState.MOVING_TO_OBJECT:
            # Mover solo M1 primero
            self.q_target_raw[0] = np.deg2rad(self.target_m1_pp)
            self.q_target_raw[1] = 0.0
            self.q_target_raw[2] = 0.0
            
            if time.time() - self.pickplace_start_time > 2.5:
                self.pickplace_state = PickPlaceState.LOWERING
                self.pickplace_start_time = time.time()
                self.get_logger().info('⬇️ Bajando...')
        
        elif self.pickplace_state == PickPlaceState.LOWERING:
            # Bajar con M2 y M3
            self.q_target_raw[0] = np.deg2rad(self.target_m1_pp)
            self.q_target_raw[1] = np.deg2rad(self.target_m2_pp)
            self.q_target_raw[2] = np.deg2rad(self.target_m3_pp)
            
            if time.time() - self.pickplace_start_time > 3.5:
            ...
            if time.time() - self.pickplace_start_time > 1.5:
            ...
            # Levantar
            self.q_target_raw[0] = np.deg2rad(self.target_m1_pp)
            self.q_target_raw[1] = np.deg2rad(self.lift_height_m2)
            self.q_target_raw[2] = np.deg2rad(self.lift_height_m3)
            
            if time.time() - self.pickplace_start_time > 3.0:
                self.pickplace_state = PickPlaceState.ROTATING_TO_BIN
                self.pickplace_start_time = time.time()
                bin_angle = self.bin_positions[self.object_class]
                self.get_logger().info(f'🔄 Rotando a {bin_angle}°...')
        
        elif self.pickplace_state == PickPlaceState.ROTATING_TO_BIN:
            # Rotar a bin
            bin_angle = self.bin_positions[self.object_class]
            self.q_target_raw[0] = np.deg2rad(bin_angle)
            self.q_target_raw[1] = np.deg2rad(self.lift_height_m2)
            self.q_target_raw[2] = np.deg2rad(self.lift_height_m3)
            ...
            if time.time() - self.pickplace_start_time > 3.5:
                ...
                self.gripper_val = 90
                self.get_logger().info('✅ Ciclo completo! Esperando siguiente objeto...')

    # ═══════════════════════════════════════════════════════
    # FUNCIÓN PARA ACTIVAR/DESACTIVAR PICK&PLACE
    # ═══════════════════════════════════════════════════════
    def toggle_pickplace(self):
        if self.pickplace_state == PickPlaceState.DISABLED:
            self.pickplace_state = PickPlaceState.IDLE
            self.motors_enabled = True  # Auto-ON
            self.get_logger().info('🟢 Pick&Place ACTIVADO')
            return True
        else:
            self.pickplace_state = PickPlaceState.DISABLED
            self.get_logger().info('🔴 Pick&Place DESACTIVADO')
            return False
...
class RobotGUI:
    ...
    def setup_ui(self):
        ...
        self.btn_power = tk.Button(ctrl_frame, text="⚡ ACTIVAR MOTORES (OFF)", command=self.toggle_power, 
                                font=('Segoe UI', 11, 'bold'), relief=tk.FLAT, width=22, height=2, cursor='hand2')
        self.btn_power.pack(side=tk.LEFT, padx=5)
        
        self.btn_test = tk.Button(ctrl_frame, text="🌊 TEST SENOIDAL", command=self.toggle_test, 
                                font=('Segoe UI', 11, 'bold'), relief=tk.FLAT, width=18, height=2, cursor='hand2')
        self.btn_test.pack(side=tk.LEFT, padx=5)
        
        # ← NUEVO BOTÓN PICK & PLACE
        self.btn_pickplace = tk.Button(ctrl_frame, text="🤖 PICK & PLACE", command=self.toggle_pickplace, 
                                    font=('Segoe UI', 11, 'bold'), relief=tk.FLAT, width=18, height=2, cursor='hand2')
        self.btn_pickplace.pack(side=tk.LEFT, padx=5)
    ...
    def toggle_pickplace(self):
        st = self.controller.toggle_pickplace()
        t = self.current_theme
        self.btn_pickplace.config(
            text="🤖 PICK&PLACE: ON" if st else "🤖 PICK & PLACE",
            bg=t['btn_pickplace'] if st else t['ps4_inactive']
        )
        if st:
            self.btn_power.config(text="⚡ MOTORES: ON", bg=t['btn_power'])
    ...
    def apply_theme(self, theme_key):
        ...
        if self.controller.pickplace_state != PickPlaceState.DISABLED:
            self.btn_pickplace.configure(bg=t['btn_pickplace'])
        else:
            self.btn_pickplace.configure(bg=t['ps4_inactive'])
    ...
    def reset_gui(self):
        ...
        self.btn_pickplace.config(text="🤖 PICK & PLACE", bg=t['ps4_inactive'])

...
class RobotGUIControllerDual(Node):
    ...
    def control_loop(self):
                ...
                # AHORA: Inicializa automáticamente en modo simulación
        if not self.arduino_connected and not self.initialized_pose:
            self.q_curr = np.zeros(3)
            self.q_target_raw = np.zeros(3)
            self.traj_gen.q = np.zeros(3)
        ...
        if self.motors_enabled:
            ...
            # Enviar comando solo si Arduino está conectado
            if self.arduino_connected:
                try: 
                    self.arduino_port.write(f"CMD:{u[0]:.2f},...\n".encode())
                except: pass
            else:  # <-- AQUÍ VA TU CÓDIGO
                # Actualizar posición actual como si fuera feedback
                self.q_curr = q_d.copy()
                self.real_M1 = np.rad2deg(q_d[0])
                self.real_M2 = np.rad2deg(q_d[1])
                self.real_M3 = np.rad2deg(q_d[2])

        # Publicar comandos a Gazebo (siempre, con o sin Arduino)
        msg_arm = Float64MultiArray(); msg_arm.data = [q_d[0], q_d[1], q_d[2]]; self.arm_pub.publish(msg_arm)

        # Publicar gripper a Gazebo (convertir de servo 70-120 a radianes)
        # 70 = abierto (0.0 rad), 120 = cerrado (~0.87 rad)
        gripper_rad = np.interp(self.gripper_val, [70, 120], [0.0, 0.87])
        msg_gripper = Float64MultiArray(); msg_gripper.data = [gripper_rad, gripper_rad]; self.gripper_pub.publish(msg_gripper)
...
class RobotGUI:
    ...
    def setup_ui(self):
        ...
        self.btn_ps4 = tk.Button(self.ps4_frame, text="🎮 CONTROL MANDO: DESACTIVADO", command=self.toggle_ps4, font=('Segoe UI', 12, 'bold'), relief=tk.FLAT, width=30, height=2, cursor='hand2'); self.btn_ps4.pack(pady=5)
    ...
    def update_ps4_btn_color(self):
        ...
        if self.controller.ps4_enabled: self.btn_ps4.config(text="🎮 CONTROL MANDO: ACTIVADO", bg=t['ps4_active'], fg='#1a1b26')
        else: self.btn_ps4.config(text="🎮 CONTROL MANDO: DESACTIVADO", bg=t['ps4_inactive'], fg='#ffffff' if self.current_theme_key == 'dark' else '#2c3e50')
    def toggle_ps4(self):
